...
function platforms(s,t){
 ...
 if(s.id===3&&!s.obstacles[0])return [{id:'bridge-detour',x:1070,y:D.mountain(1070)-95,w:165}];
 if(s.id===5)return D.racePlatforms(s,t);
 return [];
}
function floor(s,x){
 if(s.id===1&&((x>410&&x<565)||(x>1420&&x<1680)||(x>1960&&x<2170)))return 1000;
 if(s.id===3){if(!s.obstacles[0]&&x>1080&&x<1210)return 2000;return D.mountain(x);}
...
function walls(s){
 ...
 if(s.id===3&&!s.obstacles[1])return [2200];
