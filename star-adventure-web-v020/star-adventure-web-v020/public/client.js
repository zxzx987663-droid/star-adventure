...
function drawWorld(){if(!room)return;const s=room.stage,t=clientTime;if(s.id==='ending'&&s.phase==='celebrate'){camera=0;drawEndingScene(s);return;}background(s);
 ...
 if(![1,2,3,4,5,'party','ending'].includes(s.id)){let segment=[];const fillGround=()=>{if(!segment.length)return;ctx.beginPath();ctx.moveTo(segment[0][0],560+cameraY);for(const [x,y] of segment)ctx.lineTo(x,y);ctx.lineTo(segment.at(-1)[0],560+cameraY);ctx.closePath();ctx.fillStyle='#b7cc9d';ctx.fill();ctx.beginPath();segment.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));ctx.strokeStyle='#dce6b7';ctx.lineWidth=9;ctx.stroke();segment=[];};for(let x=Math.max(0,Math.floor(camera/10)*10-20);x<=Math.min(width,camera+viewW+30);x+=10){const y=W.floor(s,x);if(y<700)segment.push([x,y+4]);else fillGround();}fillGround();}
 if(s.id===1){
