...
window.LevelView={
 ...
 if(s.id===2){
  ...
  const section=(x1,x2,active=false)=>{const top=406,bottom=552,r=34,body=ctx.createLinearGradient(0,top,0,bottom);body.addColorStop(0,active?'#87978c':'#758078');body.addColorStop(1,'#515d62');ctx.beginPath();ctx.moveTo(x1+r,top);ctx.lineTo(x2-r,top);ctx.quadraticCurveTo(x2,top,x2,top+r*.4);ctx.lineTo(x2,bottom);ctx.lineTo(x1,bottom);ctx.lineTo(x1,top+r*.4);ctx.quadraticCurveTo(x1,top,x1+r,top);ctx.closePath();ctx.fillStyle=body;ctx.fill();ctx.strokeStyle=active?'#eee6ad':'#b9c49a';ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(x1+r,top);ctx.lineTo(x2-r,top);ctx.quadraticCurveTo(x2,top,x2,top+r*.4);ctx.stroke();};
  ...
  D.caves.forEach((c,i)=>{
   const caveNo=i+1,live=caveNo===s.cave&&!s.caveCleared[caveNo],cleared=!!s.caveCleared[caveNo],x1=c.start-86,x2=c.gate+118;
   section(x1,x2,live); if(i<4)section(c.gate+118,c.gate+200,false);
   ribbon(c.start+420,174,190,`洞穴 ${caveNo} ${cleared?'✓':''}`,cleared?'已通關':live?'進行中':'未解鎖');
   c.pads.forEach((x,j)=>{const key='l2Pad'+padKeys[j],state=live?'on':'off';ctx.save();if(!live&&!cleared)ctx.globalAlpha=.72;if(cleared)ctx.globalAlpha=.9;if(!Art.draw(ctx,'levels',key,state,x,418,t,.78)){ellipse(x,412,34,12,live?'#fff0c7':'#d5d3b3');text(['🌸','⭐','🍄','🌙'][j],x,414,25);}ctx.restore();if(live)ellipse(x,423,26,7,'#fff6c166');});
   const readers=caveNo>=4?2:1;
   for(let j=0;j<readers;j++){const x=c.readers[j],active=live&&s.mode==='waiting',state=caveNo>=4&&j===1?'special':'normal';if(!Art.draw(ctx,'levels','l2Reader',state,x,420,t,.96))rect(x-24,344,48,76,active?'#e5dd9e':'#83968c',8);if(active){ellipse(x,332,38,10,'#f5d78666');text('✦',x,338,24,'#ffe28b');}text(j?'B':'A',x,377,15,'#614f42');if(live)text(room.players.find(p=>p.token===s.readers[j])?.char||'',x,448,12,'#fff1cf');}
   if(!Art.draw(ctx,'levels','l2Gate',cleared?'open':'closed',c.gate,428,t,.96))gate(c.gate,cleared);
   if(cleared)for(let k=0;k<4;k++)text('✦',c.gate-26+k*16,332+Math.sin(t*5+k)*5,13,'#ffe9a6');
   else if(live){ellipse(c.gate,363,62,18,'#ffecc433');text('月光封印',c.gate,336,11,'#fff3d2');}
  ...
  section(4988,5228,gatherReady);if(!Art.draw(ctx,'levels','l2Gate',gatherReady?'open':'closed',5100,428,t,1.02))gate(5100,gatherReady);if(gatherReady)for(let k=0;k<6;k++)text('✦',5050+k*18,325+Math.sin(t*5+k)*5,14,'#ffe9a6');ribbon(5100,222,180,'六位集合',gatherReady?'出口已開啟':'完成五個洞穴');text('⭐',5100,396,44,'#ffe49b');
 }
 if(s.id===3){const terrain=()=>{const yAt=x=>D.mountain(x)+4;const g=ctx.createLinearGradient(0,180,0,560);g.addColorStop(0,'rgba(110,128,116,.94)');g.addColorStop(1,'rgba(83,94,98,.98)');ctx.beginPath();ctx.moveTo(0,560);ctx.lineTo(0,yAt(0));for(let x=0;x<=5600;x+=12)ctx.lineTo(x,yAt(x));ctx.lineTo(5600,560);ctx.closePath();ctx.fillStyle=g;ctx.fill();ctx.beginPath();ctx.moveTo(0,yAt(0));for(let x=0;x<=5600;x+=16)ctx.lineTo(x,yAt(x));ctx.strokeStyle='#e8dfaf';ctx.lineWidth=8;ctx.stroke();ctx.beginPath();ctx.moveTo(0,yAt(0)-2);for(let x=0;x<=5600;x+=18)ctx.lineTo(x,yAt(x)-2);ctx.strokeStyle='#9fbc84';ctx.lineWidth=4;ctx.stroke();for(let x=120;x<5480;x+=170){const y=yAt(x);Art.draw(ctx,'levels','l3Platform','idle',x,y+18,t,.88);}for(let x=160;x<5400;x+=240){ctx.strokeStyle='rgba(86,111,92,.55)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(x,yAt(x)+6);ctx.lineTo(x,yAt(x)+18+((x/37)|0)%12);ctx.stroke();}};terrain();for(const b of World.platforms(s,t)){const sc=Math.max(.75,Math.min(1.45,b.w/110));if(!Art.draw(ctx,'levels','l3Platform','idle',b.x+b.w/2,b.y+18,t,sc))rect(b.x,b.y,b.w,14,'#ab9488',4);}sign('🌕 推月亮任務',400,240);sign('3 人推 · 放手會倒滾',660,180);sign('前跑組 ↑ 跳上高台按 E',1130,100);sign('下坡！放手，小心月亮！',2770,100);
 ...
 const buttonPedestal=(x,done,char,i)=>{rect(x-22,418,44,18,done?'#d9c47b':'#7788a3',7);rect(x-15,433,30,36,done?'#94704f':'#5e6883',4);ellipse(x,413,16,7,done?'#fff0a7':'#d7deea');ellipse(x,408,21,10,done?'#ffd777':'#96a7c7');text(done?'★':'E',x,414,done?13:11,done?'#af7620':'#4b5975');text(char||'',x,378-i%2*16,10,'#fff2dc');if(done)for(let k=0;k<3;k++)text('✦',x-18+k*18,384+Math.sin(i+k+t*6)*4,10,'#ffe9a2');};
 ...
 s.participants.forEach((token,i)=>{const p=room.players.find(p=>p.token===token),x=D.boss.buttons[i],done=s.completed.includes(token);buttonPedestal(x,done,p?.char,i);});sign('各自按鈕 · E',3580,233);text(`${s.completed.length} / 6`,3580,313,28,'#fff0c5');if(s.bridgeOpen)text('✦ 月光機關啟動！ ✦',2000,325,22,'#ffe69b');
 }
 if(s.id===5){
 ...
 const drawGroundSeg=(x1,x2,y)=>{const g=ctx.createLinearGradient(0,y,0,560);g.addColorStop(0,'rgba(95,104,132,.96)');g.addColorStop(1,'rgba(74,83,111,.98)');ctx.beginPath();ctx.moveTo(x1,560);ctx.lineTo(x1,y+4);for(let x=x1;x<=x2;x+=18)ctx.lineTo(x,y+4+Math.sin(x*.013)*1.5);ctx.lineTo(x2,560);ctx.closePath();ctx.fillStyle=g;ctx.fill();ctx.beginPath();ctx.moveTo(x1,y+3);for(let x=x1;x<=x2;x+=18)ctx.lineTo(x,y+3+Math.sin(x*.013)*1.3);ctx.strokeStyle='#f0e4aa';ctx.lineWidth=8;ctx.stroke();ctx.beginPath();ctx.moveTo(x1,y+1);for(let x=x1;x<=x2;x+=18)ctx.lineTo(x,y+1+Math.sin(x*.013)*1.1);ctx.strokeStyle='#a9c783';ctx.lineWidth=4;ctx.stroke();for(let x=x1+90;x<x2-80;x+=190){ellipse(x,y+15,38,7,'rgba(255,240,196,.15)');ctx.strokeStyle='rgba(101,122,116,.45)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(x,y+7);ctx.lineTo(x,y+26+((x/41)|0)%14);ctx.stroke();}};[[0,540],[730,1080],[1280,1800],[2070,2450],[2720,3300],[3570,3950],[4200,4850],[5140,5700]].forEach(([a,b])=>drawGroundSeg(a,b,D.raceFloor((a+b)/2)));
