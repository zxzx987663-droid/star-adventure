...
app.get('/health',(req,res)=>res.json({ok:true,service:'star-adventure',version:'0.3.0'}));
...
function updateStage(r){
  ...
  if(n>=3&&!r.stage.gateOpen){r.stage.gateOpen=true;io.to(r.code).emit('toast','三個壓力板同時亮了！門打開了！');}
  if(!r.stage.bridgeBroken&&[...r.players.values()].some(p=>p.x>1450)){r.stage.bridgeBroken=true;io.to(r.code).emit('toast','喀啦……橋是不是有點不妙？');}
  if(r.stage.leverOn)r.stage.checkpoint=1780;
}
...
io.on('connection',s=>{
  ...
  s.on('move',pos=>{const {r,p}=getPlayer(s);if(!r||!p||p.down)return;const x=Math.max(0,Math.min(2580,Number(pos?.x)||0)),y=Math.max(0,Math.min(700,Number(pos?.y)||0));p.x=x;p.y=y;if(p.token===r.hostToken)botAssist(r,p);updateStage(r);s.to(r.code).emit('move',{token:p.token,x,y});emit(r);});
...
const PORT=process.env.PORT||3000;server.listen(PORT,()=>console.log(`Star Adventure Web v0.3 listening on ${PORT}`));
